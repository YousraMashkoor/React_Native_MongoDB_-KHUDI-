...
class Article extends Component {
  ...
  renderDots = () => {
    ...
    const article = this.props.navigation.navigate('article');
    ...
    // ) */}
        ...
        <View style={[styles.flex, styles.content]}>
          <View style={[styles.flex, styles.contentHeader]}>
            <Image style={[styles.avatar, styles.shadow]} source={{ uri: this.props.route.params.img }} />
            <Text style={styles.title}>{this.props.route.params.name}</Text>
            <View style={[
              styles.row,
              { alignItems: 'center', marginVertical: theme.sizes.margin / 2 }
            ]}>
              {this.renderRatings(this.props.route.params.rating)}
              <Text style={{ color: theme.colors.active }}>
                {this.props.route.params.rating} 
              </Text>
              <Text style={{ marginLeft: 8, color: theme.colors.caption }}>
                 ({this.props.route.params.type} reviews)
              </Text>
            </View>
            <TouchableOpacity>
              <Text style={styles.description}>
                {this.props.route.params.type.split('').slice(0, 180)}...
